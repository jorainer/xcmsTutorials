...
![docker
pulls](https://img.shields.io/docker/pulls/jorainer/xcms_tutorials)[![Docker
Image Version (latest by
date)](https://img.shields.io/docker/v/jorainer/xcms_tutorials?label=docker%20image)](https://hub.docker.com/repository/docker/jorainer/xcms_tutorials)
[![DOI](https://zenodo.org/badge/645255717.svg)](https://zenodo.org/doi/10.5281/zenodo.11185520)

![xcms](reference/figures/xcms.png)![MsExperiment](reference/figures/MsExperiment.png)![Spectra](reference/figures/Spectra-rainbow.png)![MetaboCoreUtils](reference/figures/MetaboCoreUtils.png)![MsCoreUtils](reference/figures/MsCoreUtils.png)

This workshop provides an overview of recent developments in
Bioconductor to work with mass spectrometry data
([MsExperiment](https://github.com/RforMassSpectrometry/MsExperiment),
[Spectra](https://github.com/RforMassSpectrometry/Spectra)) and
specifically LC-MS data ([xcms](https://github.com/sneumann/xcms)) and
walks through the preprocessing of a small data set emphasizing on
selection of data-dependent settings for the individual preprocessing
steps.
...
provided in the
[xcms-preprocessing.Rmd](https://jorainer.github.io/xcmsTutorials/vignettes/xcms-preprocessing.Rmd)
file. This file can be opened with e.g. RStudio which allows execution
of the individual R commands (see section below for additionally
required R packages). The R command
`rmarkdown::render("xcms-preprocessing.Rmd")` would generate the html
file
...
The workshop files along with an R runtime environment including all
required packages and the RStudio (Posit) editor are all bundled in a
*docker* container. After installation, this docker container can be run
on the computer and the code and examples from the workshop can be
evaluated within this environment (without the need to install any
...
- In the RStudio server version: open any of the R-markdown (*.Rmd*)
