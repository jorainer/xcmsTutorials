...
infrastructure as it provides more flexibility and a higher performance.
...
also be possible to import MS data from other files than mzML, mzXML or CDF
files.
...
[SpectraTutorials](https://jorainer.github.io/SpectraTutorials). Besides the
peak data (*m/z* and intensity values) also additional spectra variables
(metadata) are available in a `Spectra` object. These can be listed using the
`spectraVariables` function that we call on our example MS data below. To avoid
nested function calls and hence improve the readability of the code, we use the
R pipe operator `|>` that allows to concatenate consecutive calls in a more
readable fashion.
...
`MsExperiment` to which it belongs. Below we subset our object to the first data
file, extract the retention times of the spectra from that file and, by using
the `head` function, display the first 6 of them.
...
Alternatively, the `peaksData` function could be used to extract the *m/z* and
intensity values of a spectrum as a (two column) numeric matrix.
...
(`MsExperiment`) or individual data files (`Spectra`). TODO: ADD REFERENCE TO
MSQUALITY PAPER.
...
at an *m/z* of about 130 seconds and the second largest at about 106, which
...
```{r chromatogram}
#' get intensity valuesfor the chromatogram of the first sample
...
the data or of randomly selected chromatographic peaks. To illustrate this we
define below a matrix with *m/z* and retention time ranges for a set of
(pre)selected peaks. The same matrix could also be created either by randomly
selecting peaks or by defining it manually. For our example we further expand
...
to properly detect a peak). In all cases, however, signal from (presumably)
the same ion was measured in both samples. Thus, reporting a missing value would
...
which allows to configure this algorithm. Below we perform this gap filling
summing also the number of missing values before and after running
`fillChromPeaks`.
...
```{r gap-filling-missing, fig.cap = "Features with missing values even after gap-filling."}
...
A nice way to check that gap-filling was able to correctly rescue signals is to
plot, for features with at least one missing value, the average detected against
the average filled-in signal. Ideally, this should be done on QC samples or
other repeatedly measured samples were no difference in feature abundance
...
filled-in signal in which the detected signal is subsequently replaced with
`NA`. Finally, after calculating the row averages for both matrices (excluding
missing values), the values are plotted against each other.
...
- don't use defaults of *xcms*.
- use the infrastructure to inspect and summarize the data.
- don't limit yourself to just *xcms*, use the vast number of additional
  packages, methods, visualizations in R.
- build custom workflows, data visualization approaches or your own analysis
  method.
...
also to evaluate the performance (and quality) of a preprocessing. LC-MS data is
...
provide some additional examples how such data can be visualized. In particular,
we show an alternative way to evaluate and investigate identified
chromatographic peaks in a data file.

We below subset the data to the first sample and visualize the identified
chromatographic peaks in the *m/z* - retention time plane.
...
While this plot provides some information on the region in which peaks were
...
sample. With parameter `expandRt = 4` we extract also signal measured 2 seconds
...
We could now plot each of the `r length(chrs_all)` EICs separately (using the
`plot` function). As an alternative, we can use the `plotChromatogramsOverlay`
function from the *xcms* package to plot multiple EICs into the same plot. By
setting parameter `stacked` to a value different then `0` it is possible to
*stack* the chromatograms along the y-axis hence providing a 3-dimensional
impression of the data. For easier visualization we in addition increase the
transparency of the colors for the individual lines (parameter `col`), and the
identified chromatographic peaks (parameter `peakCol` and `peakBg` for the
...
Instead of the full data range we can also extract EICs from selected regions of
the data. Below we define an area from a *m/z* value from 113 to 119 and a
retention time from 125 to 145 seconds.
...
We next extract chromatographic peaks from that region, define *m/z* and
retention time ranges from that and extract ion chromatograms for these slices.
...
We can now plot the EICs from that region with the
`plotChromatogramsOverlay`. This function also silently returns the y-position
of the individual EICs in the plot which we assign to a variable `y` and use
that information to indicate the *m/z* for the EICs along the y-axis.
...
and 115.07).  The infrastructure around *xcms* thus allows to create customized
visualizations of the data. Note however, that the plots created above only
showed **detected** chromatographic peaks in a sample but not the *full* MS data
(i.e. signals outside of identified chromatographic peaks).
...
tmp <- mass2mz(123, adductNames())

diffs <- outer(tmp[1, ], tmp[1, ], "-")
diffs[lower.tri(diffs, TRUE)] <- NA
diffs <- abs(diffs)

pk_diffs <- outer(pks[, "mz"], pks[, "mz"], "-")
pk_diffs[lower.tri(pk_diffs, TRUE)] <- NA
pk_diffs <- abs(pk_diffs)

res <- outer(pk_diffs, diffs, "-")
