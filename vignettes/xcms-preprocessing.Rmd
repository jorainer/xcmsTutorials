...
described *xcms*-based data preprocessing.

Besides the peak data (*m/z* and intensity values) also additional spectra
variables (metadata) are available. These can be listed using the
`spectraVariables` function. , for example by listing all available spectra
variables using the `spectraVariables` function. To avoid nested function calls
...
being samples (files) and rows data slices (i.e. *m/z* - rt ranges). This type
of data representation is likely to be replaced in future with a more efficient
and flexible data structure similar to `Spectra`. Data from the individual
