...
Many ions will result, when measured with MS alone, in a very similar m/z. Thus, making
it difficult or impossible to discriminate them. MS is therefore frequently coupled
...
Phenotype information can be retrieved with the `sampleData` function
from that object.
...
As an example we below calculate the retention time range of the
...
To access MS data from only a single file (sample) we need to subset the 
`MsExperiment` object to that particular sample before extracting the spectra 
data with `spectra`. Below we subset the data to the second sample using the `[`
function, access the spectra from that sample, extract their retention times and
...
Centroiding is the process to reduce these mass peaks to a single representative
signal, the centroid. This results in much smaller file sizes, without loosing 
too much information. *xcms*, specifically the *centWave* chromatographic peak 
detection algorithm, was designed for centroided data, thus, prior to data 
...
Some spectra will have more than one peak in the MS data subset. For these
...
overlap in a 4 seconds window if the signal between them is lower than
...
The signal is rather wide and noisy, thus *centWave* was not able to integrate 
the whole peak and even defined partially overlapping chromatographic peaks 
(left in the image above). Peak refinement merged all the consecutive peaks as 
the signal between them never dropped below 75% of the smallest chromatographic 
peak's intensity.
...
In a similar way as before, we set `chromPeaks = "none"` in the `chromatogram` 
call to tell the function to **not** include any identified chromatographic 
peaks in the returned chromatographic data.
...
The BPCs are nicely aligned after retention time adjustment. The impact of the 
alignment should also be evaluated on known compounds or internal standards. 
We thus plot below the EIC for serine before and after alignment. To get raw 
retention times and hence be able to extract the ion chromatogram of serine 
before alignment we have to *drop* the alignment results which can be done with 
the `dropAdjustedRtime` function. This function will restore the retention 
...
```{r serine-EIC-adjusted, fig.cap = "EIC for Serine before (left) and after (right) alignment", fig.width = 10, fig.height = 4}
...
to avoid peaks from different ions with similar m/z and retention time
...
have the same dimensions). For example we can add the feature quantification 
excluding filled-in signal as an additional assay matrix:
...
We now have two assay matrices in our result object:
